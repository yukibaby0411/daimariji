...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('tel')->unique();                                    //手机
            $table->string('email')->unique()->nullable();                      //邮箱
            $table->string('password');                                         //密码
            $table->integer('pass_strong');                                     //密码强度
            ...
            $table->string('activation_token')->nullable();                   //邮箱会话验证码
            $table->integer('activations')->default(2017012800);                       //短信账户状态
